<x-app-layout>
    @php
        ...
        $sensorMeta = [
            ['key' => 'rpm', 'label' => 'RPM', 'unit' => 'A', 'decimals' => 2],
            ['key' => 'hm', 'label' => 'HM', 'unit' => '', 'decimals' => 2],
            ['key' => 'temp', 'label' => 'TEMP', 'unit' => '°C', 'decimals' => 0],
            ['key' => 'humi', 'label' => 'HUMI', 'unit' => '%', 'decimals' => 0],
        ];
    @endphp
    ...
    <div class="content-container">
        ...
        <div id="reportDownloadModal" class="modal-overlay custom-range-modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="reportForm" action="{{ route('rpm.download-report') }}" method="GET">
                        <div class="date-picker-group">
                            <div class="date-picker-field">
                                <label><i class="fa-solid fa-calendar-check"></i> Start Date & Time</label>
                        ...
                        <thead>
                            <tr>
                                <th class="cursor-pointer hover:bg-slate-50 transition" onclick="toggleSort('name')">
                                    No. RPM <i class="fa-solid fa-sort text-slate-300 ml-1" id="sort-icon-name"></i>
                                </th>
                        ...
                        <tbody id="machinesTableBody">
                            @foreach ($machines as $m)
                                <tr class="border-b border-slate-100">
                                    <td data-label="No. RPM" class="px-4 py-3 font-semibold text-slate-700">
                                        {{ $m['name'] ?? '-' }}</td>
                                    ...
                                    @foreach ($sensorMeta as $s)
                                        ...
                                        <td data-label="{{ $s['label'] }}"
                                            ...
                                            @if (in_array($s['key'], ['temp', 'humi', 'rpm', 'hm']) && (float) $v == 0)
                                                -
                                            @else
    ...
    @push('scripts')
        ...
        <script>
            // Endpoint tabel & grid
            const TABLE_ENDPOINT = "{{ route('rpm.table-data') }}";
            const GRID_ENDPOINT = "{{ route('rpm.get-data') }}";
                ...
                pageRows.forEach(m => {
                    const sensorTds = tableSensorMeta.map(s => {
                        ...
                        if (ok) {
                            ...
                            if (
                                v === 0 ||
                                ((s.key === 'temp' || s.key === 'humi' || s.key === 'rpm' || s.key === 'hm') &&
                                    Math.round(v) === 0)
                            ) {
                                txt = '-';
                            } else {
                    ...
                    html += `
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                            <td data-label="No. RPM" class="px-4 py-3 font-semibold text-slate-700">${esc(m.name ?? '-')}</td>
