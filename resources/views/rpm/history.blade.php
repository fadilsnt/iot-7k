<x-app-layout>
    ...
    <header class="dashboard-header"
        ...
        <h1 style="font-size: 1.25rem; margin-top: 0.5rem;">Riwayat Performa {{ $rpm_id }}</h1>
        <a href="{{ route('rpm.index') }}"
            class="absolute left-6 top-1/2 -translate-y-1/2 flex items-center justify-center w-8 h-8 rounded-full bg-white/20 text-white hover:bg-white/30 transition">
            ...
            <div class="table-card" style="grid-column: 1 / -1;">
                <div class="table-header-controls"
                    ...
                    <div class="table-controls"
                        ...
                        <button class="btn-custom-range btn-custom-range-pill" id="btnDownloadExcel"
                            style="border-color: #4f76ff; color: #4f76ff; box-shadow: 0 8px 18px rgba(79, 118, 255, 0.12);">
                    ...
                    <table id="historyDataTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-slate-500">RPM</th>
                                <th class="text-slate-500">HM</th>
                                <th class="text-slate-500">TEMP</th>
                                <th class="text-slate-500">HUMI</th>
                            </tr>
                        </thead>
                ...
                <div id="historyPagination"
                    class="px-4 py-3 text-sm text-slate-600 flex items-center justify-between gap-3"
    ...
    @push('scripts')
        ...
        <script>
            const DATA_ENDPOINT = "{{ route('rpm.history-data', ['rpm_id' => $rpm_id]) }}";
            const EXPORT_ENDPOINT = "{{ route('rpm.history-export', ['rpm_id' => $rpm_id]) }}";
            ...
            jQuery(document).ready(() => {
                ...
                jQuery('#btnCloseModal, #btnCancel').on('click', () => jQuery('#customRangeModal').removeClass(
                    'active'));
                ...
                jQuery('#btnApply').on('click', () => {
                    ...
                    currentPage = 1;
                    jQuery('#btnHistoryCustom').html(
                        `<i class="fa-regular fa-calendar-check text-xs mr-1"></i> ${start} - ${end}`);
                ...
                jQuery.ajax({
                    ...
                    .fail((xhr) => {
                        ...
                        const msg = xhr.responseJSON ? (xhr.responseJSON.message || xhr.responseJSON.error) :
                            'Gagal memuat data (Timeout/Jaringan)';
                        jQuery('#historyTableBody').html(
                            `<tr><td colspan="5" class="px-4 py-8 text-center text-red-500 font-semibold">${msg}</td></tr>`
                            );
            ...
            function renderTable(rows) {
                ...
                if (!rows || rows.length === 0 || (rows.length === 1 && rows[0].last_updated === '-')) {
                    html =
                        '<tr><td colspan="5" class="px-4 py-8 text-center text-slate-500">Tidak ada data untuk rentang waktu ini</td></tr>';
                } else {
                    rows.forEach(m => {
                        html += `
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                            ...
                            ${tableSensorMeta.map(s => {
                                ...
                                return ` < td data - label = "${s.key.toUpperCase()}"
